...
class Cell:
    ...
    def get_bar_text(self, max_val, current_val, max_size) -> str:
        if current_val > max_val:
          return Cell.over
        bar = '|'
        bar += ("=" if (int(current_val)>=round(float(max_val))) else "#") * floor((float(current_val) / float(max_val)) * max_size)
    ...
    def get_cell_text(self) -> str:
        ...
        return [self.center_text(dict["Hostname"][0:self.width].upper())+' ',
...
"""]
    ...
    def center_text(self, some_str) -> str:
        centered_str = " " * int((self.width - len(some_str)) / 2) + some_str + " "*int((self.width - len(some_str)) / 2)
        return centered_str
...
def get_bash_cinfo_as_com(fl) -> Computer:
    df = pd.read_csv(fl, header=None, index_col=0, squeeze=True)
    info = df.to_dict()
    return Computer(info["alive"], info["hostname"], info["address"], info["max_ram"], info["cpu"], info["cur_ram"],
            info["task1_name"], info["task2_name"], info["task3_name"],
            info["task1_cpu"], info["task2_cpu"], info["task3_cpu"])
...
def main(stdscr):
    germain = get_bash_cinfo_as_com("data.csv")
    germain_cell = new_cell(germain, 0, 0, _17X15)

    curses.init_pair(1, (curses.COLOR_BLACK if germain.dictionary()["Alive"] else curses.COLOR_WHITE), (curses.COLOR_WHITE if germain.dictionary()["Alive"] else curses.COLOR_RED))
    
    stdscr.clear()
    while True:  #change later to while button hit is not escape or control + c or something like that
      cell_text = germain_cell.get_cell_text()
